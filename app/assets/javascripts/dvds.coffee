    ...
    if (window.location.pathname.split('/')[window.location.pathname.split('/').length - 1] != 'edit')
      $.ajax({
        ...
        if (data.status == 'success')
          $('#dvd_abstract_source').val(data.omdb.abstract_source)
          $('#dvd_abstract_txt').val(data.omdb.abstract_txt).height($('#dvd_abstract_txt').prop('scrollHeight'))
          $('#dvd_abstract_url').val(data.omdb.abstract_url)
          $('.image_to_upload').attr('src', data.omdb.image_url)
          $preview = $('.preview')
